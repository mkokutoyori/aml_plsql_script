...
--   The money market module stops on 16/06/2025. The business then moved to
--   CALYPSO, which posts entries straight into the general ledger and creates
--   no contract, so not one of the 56 controls below can be run on it: they
--   all confront the entries with the terms of a deal file that does not
--   exist. The paradigm is not the same and the two are not mixed. Calypso is
--   audited by its own script, audit_calypso.sql.
--
-- SCOPE CONVENTION
...
BEGIN
    ...
    BEGIN
        ...
        po('');
        po('  One family has been added beyond the matrix.');
        po('');
        ...
        po('  9. This script stops where the money market module stops. The');
        po('     business moved to CALYPSO on 16/06/2025, and Calypso posts entries');
        po('     without creating any contract, so not one control here can be run');
        po('     on it. It is audited separately by audit_calypso.sql. Nothing in');
        po('     this report says anything about the securities bought or sold');
        po('     after that date.');
