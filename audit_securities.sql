...
--   PART 12 - THE CALYPSO INTERFACE      CAL-01 to CAL-13
--             12.1 to 12.4   footprint, calendar, narrative, accounts moved
--             12.5           THE BRIDGE ACCOUNTS AND THEIR RESIDUAL BALANCE
--             12.6           THE PORTFOLIO REBUILT FROM THE ENTRIES
--             12.7 to 12.10  engine, revaluation, calendar, blind spots
--   PART 13 - SUMMARY OF ALL TESTS
--
-- SCOPE CONVENTION
...
DECLARE
    ...
    -- ========================================================================
    -- THE CALYPSO INTERFACE (part 12)
    -- Calypso is the front office system that replaced the money market
    -- module. It does not create contracts in FLEXCUBE: it posts accounting
    ...
    -- If section 12.3 shows the fields shifted by one, because the string
    ...
    -- the interface has not closed, and is the subject of section 12.5.
    ...
    v_cy_nb   NUMBER := 0;    -- Calypso entry lines in scope, reference denominator
    v_cy_mt   NUMBER := 0;    -- gross Calypso flow in scope
    v_cy_dl   NUMBER := 0;    -- distinct Calypso deal keys in scope
    v_cy_d1   DATE;           -- first Calypso entry
    v_cy_d2   DATE;           -- last Calypso entry
    v_d_last2 DATE;           -- second date buffer for the Calypso cross checks
    ...
    -- EXTERNAL_REF_NO. Same purpose as sec_head for the money market module:
    -- a finding nobody can trace to a named item is not actionable.
    -- ========================================================================
    ...
    BEGIN
        ...
        po('     reliable as section 12.3 shows that narrative to be.');
...
BEGIN
    ...
    -- ########################################################################
    print_part('PART 12 : THE CALYPSO INTERFACE');
    -- ########################################################################
    po('');
    po('  Everything before this part concerns the money market module, whose');
    po('  last accounting entry is dated ' || fdt(v_d_last) || '. The securities business did not');
    po('  stop on that day: it moved to CALYPSO, the front office system, which');
    po('  since ' || fdt(k_cy_from) || ' posts straight into the general ledger.');
    po('');
    po('  WHAT CALYPSO SENDS, AND WHAT IT DOES NOT');
    po('');
    po('  Calypso does not create contracts in FLEXCUBE. It creates NO row in');
    po('  LDTB_CONTRACT_MASTER, so there is no nominal, no rate, no value date and');
    po('  no maturity date to test against. Everything this part can say is');
    po('  deduced from the accounting entries and from nothing else. That is not a');
    po('  weakness of the method, it is the rule of this report applied to a');
    po('  system that sends only entries: ACTB_HISTORY tells the truth, and here');
    po('  it is the only one talking.');
    po('');
    po('  All its entries carry the same four values, which is how they are');
    po('  identified:');
    po('');
    po('    MODULE      ' || k_cy_mod || '            USER_ID and AUTH_ID   ' || k_cy_user);
    po('    PRODUCT     ' || k_cy_prod || '          TRN_CODE              ' || k_cy_code);
    po('    AMOUNT_TAG  ' || k_cy_tag || '       period                ' || fdt(k_cy_from)
       || ' to ' || fdt(k_cy_to));
    po('');
    po('  One product, one tag, one transaction code for every line. FLEXCUBE');
    po('  carries no product, event or instrument granularity whatsoever. The only');
    po('  place the business information survives is the narrative');
    po('  EXTERNAL_REF_NO, and section 12.3 measures exactly how much of it can be');
    po('  relied upon before any other section uses it.');
    po('');
    po('  THE BRIDGE ACCOUNT MECHANISM, WHICH GOVERNS EVERYTHING ELSE');
    po('');
    ...
    po('  one uses a BRIDGE account as its counter leg. The bridge is the plumbing');
    po('  between the two systems: one event debits it, another clears it.');
    po('');
    ...
    po('');
    po('  A transit account must return to nil once every deal is settled. What it');
    po('  still carries is the accounting measure of what the interface has NOT');
    po('  closed. Section 12.5 is built entirely on that idea, and it is the most');
    po('  important section of this part.');
    po('');
    po('  Section 12.6 then rebuilds the SECURITIES PORTFOLIO from the entries');
    po('  alone: what the bank holds, at what carrying value, with how much');
    po('  accrued interest and how much unearned income still parked. Since no');
    po('  contract exists, that reconstruction is the only portfolio there is.');

    print_section('12. CALYPSO : WHAT THE INTERFACE POSTED, AND WHAT IT LEFT OPEN');
    BEGIN

        -- =====================================================
        print_sub('12.1 Footprint of the interface');
        ...
        IF v_cy_nb = 0 THEN
            ...
            po('  the period is wrong. Check section 12.1 a before reading further:');
        ...
        print_sub('12.1 a. Is the signature of the interface exclusive');
        ...
        print_sub('12.1 b. The interface by branch, currency, event and direction');
        ...
        -- =====================================================
        print_sub('12.2 The posting calendar, month by month');
        ...
        -- =====================================================
        print_sub('12.3 The narrative, and how far it can be trusted');
        ...
        print_sub('12.3 a. The business lines Calypso runs, read from the narrative');
        ...
        print_sub('12.3 b. The events Calypso posts');
        ...
        po('  moving billions gross. That contrast is the subject of 12.7.');
        ...
        -- =====================================================
        print_sub('12.4 Every account Calypso moves');
    ...
    -- =========================================================
    -- 12 BIS. THE BRIDGE ACCOUNTS AND WHAT THEY STILL CARRY
    -- =========================================================
    print_section('12 BIS. THE BRIDGE ACCOUNTS AND WHAT THEY STILL CARRY');
    BEGIN
        ...
        -- -----------------------------------------------------
        print_sub('12.5 a. How much each bridge still carries at ' || fdt(k_cy_to));
        ...
        -- -----------------------------------------------------
        print_sub('12.5 b. Is the residual growing, month by month');
        ...
        -- -----------------------------------------------------
        print_sub('12.5 c. How old the unmatched items are');
        ...
        -- -----------------------------------------------------
        print_sub('12.5 d. The unmatched items that carry the most, named');
        ...
        -- -----------------------------------------------------
        print_sub('12.5 e. The other residual accounts of the interface');
        ...
        po('                   by deal, which is what 12.5 d makes possible.');
    ...
    -- =========================================================
    -- 12 TER. THE SECURITIES PORTFOLIO DEDUCED FROM THE ENTRIES
    -- =========================================================
    print_section('12 TER. THE SECURITIES PORTFOLIO DEDUCED FROM THE ENTRIES');
    BEGIN
        ...
        -- -----------------------------------------------------
        print_sub('12.6 a. The portfolio at ' || fdt(k_cy_to) || ', line by line');
        ...
        -- -----------------------------------------------------
        print_sub('12.6 b. The portfolio month by month');
        ...
        -- -----------------------------------------------------
        print_sub('12.6 c. The portfolio line by line, security by security');
        ...
        po('  These totals must agree with 12.6 a. Where they do not, the narrative');
        ...
        -- -----------------------------------------------------
        print_sub('12.6 d. Accrued interest, and whether it is ever collected');
        ...
        -- -----------------------------------------------------
        print_sub('12.6 e. What is still committed off the balance sheet');
        ...
        -- -----------------------------------------------------
        print_sub('12.6 f. What the book earned and what it cost');
        ...
        po('  ones that mean anything: section 12.7 shows why the gross totals of');
    ...
    -- =========================================================
    -- 12 QUATER. THE ENGINE, THE CALENDAR AND THE REMAINING CONTROLS
    -- =========================================================
    print_section('12 QUATER. THE ENGINE, THE CALENDAR AND THE REMAINING CONTROLS');
    BEGIN

        -- -----------------------------------------------------
        print_sub('12.7 The post and reverse engine, and why gross totals lie');
        ...
        -- -----------------------------------------------------
        print_sub('12.8 The revaluation of the portfolio');
        ...
        -- -----------------------------------------------------
        print_sub('12.9 The calendar, the four eyes and the value dates');
        ...
        -- -----------------------------------------------------
        print_sub('12.10 What the entries cannot say, and what to ask Calypso');
        ...
        po('     as 12 TER shows, not internally consistent. Ask for the documented');
        ...
        po('  7. THE NARRATIVE CONTRACT. Section 12.3 measures how many formats are');
    ...
    -- ########################################################################
    print_part('PART 13 : SUMMARY OF ALL TESTS');
    -- ########################################################################
    ...
    BEGIN
        ...
        print_sub('13.1 Headline counts');
        ...
        print_sub('13.2 Findings by severity');
        ...
        print_sub('13.3 The ten findings carrying the largest exposure');
        ...
        print_sub('13.4 Coverage of the control matrix');
        ...
        po('');
        po('  Two families have been added beyond the matrix.');
        po('');
        po('  CPN, four controls in part 6 BIS. They rebuild the coupon from the');
        po('  principal, the rate and the days held, under every day count');
        po('  convention, instead of taking MAIN_COMP_AMOUNT as given. Nothing in');
        po('  the original matrix challenged that column, so a deal whose interest');
        po('  was captured wrong would have passed every test with books that agreed');
        po('  with the wrong figure.');
        po('');
        po('  CAL, thirteen controls in part 12. The matrix was written for the');
        po('  money market module, whose last entry is dated ' || fdt(v_d_last) || '. The business');
        po('  moved to Calypso, which posts entries and creates no contract, so not');
        po('  one of the 56 references can be run on it: they all need a deal file');
        po('  that does not exist. The CAL family tests what can be tested without');
        po('  one, and the bridge account residual of section 12.5 is the heart of');
        po('  it.');
        po('');
        ...
        FOR r IN (
            SELECT n, fam, obj, nb, prt, closed FROM (
                ...
                SELECT 12, 'STA', 'Static data, duplicates and limits',
                       '5', '11', 'yes, ALM limit to be set' FROM DUAL UNION ALL
                SELECT 13, 'CAL', 'The Calypso interface, entries only',
                       '13 added', '12', 'partly, Calypso report needed' FROM DUAL
            ) ORDER BY n
        ) LOOP
        ...
        SELECT COUNT(*), NVL(SUM(c.lcy_amount), 0) INTO v_cnt, v_mt
         ...
         WHERE c.module = k_mod
           ...
           AND NOT EXISTS (SELECT 1 FROM actb_history h
                            WHERE h.trn_ref_no = c.contract_ref_no
                              AND h.module = k_mod);
        print_kv('Calypso period reviewed (part 12)',
                 fdt(k_cy_from) || ' to ' || fdt(k_cy_to));
        print_kv('Calypso entry lines',           fnum(v_cy_nb)
                 || '   ' || fnum(v_cy_dl) || ' deal keys');
        print_kv('Contracts with no accounting entry (sections 1.13 to 1.17)',
                 fnum(v_cnt) || '   nominal ' || fmio(v_mt));

        print_sub('13.6 Limitations of the review');
        ...
        po('  9. Part 12 covers CALYPSO, which posts entries and creates no');
        po('     contract. Nothing there can be reconciled to a deal file, because');
        po('     none is sent: the portfolio of section 12.6 IS the portfolio, not a');
        po('     control against one. The business information lives in the free');
        po('     text of EXTERNAL_REF_NO, and section 12.3 measures how far that');
        po('     text can be trusted before anything else uses it. The Calypso');
        po('     review is cut off at ' || fdt(k_cy_to) || ' and its residual balances are read at');
        po('     that date.');
        po(' 10. The CPN controls of part 6 BIS retain, for each contract, the day');
        ...
        po(' 11. Operations are attached to the LAST VERSION of their contract.');
