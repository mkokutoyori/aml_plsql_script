...
BEGIN
    ...
    -- *** ENRICHISSEMENT : Détail des sources de revenus ***
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('  [Détail des sources de revenus]');
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE SALARY_INCOME IS NOT NULL AND SALARY_INCOME > 0;
    print_kv('  Avec SALARY_INCOME > 0', TO_CHAR(v_count));
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE RENTAL_INCOME IS NOT NULL AND RENTAL_INCOME > 0;
    print_kv('  Avec RENTAL_INCOME > 0', TO_CHAR(v_count));
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE INVESTMENT_INCOME IS NOT NULL AND INVESTMENT_INCOME > 0;
    print_kv('  Avec INVESTMENT_INCOME > 0', TO_CHAR(v_count));
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE BUSINESS_INCOME IS NOT NULL AND BUSINESS_INCOME > 0;
    print_kv('  Avec BUSINESS_INCOME > 0', TO_CHAR(v_count));
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE PROF_BUSINESS_INCOME IS NOT NULL AND PROF_BUSINESS_INCOME > 0;
    print_kv('  Avec PROF_BUSINESS_INCOME > 0', TO_CHAR(v_count));
    SELECT COUNT(*) INTO v_count FROM STTM_KYC_RETAIL WHERE OVERSEAS_INCOME IS NOT NULL AND OVERSEAS_INCOME > 0;
    print_kv('  Avec OVERSEAS_INCOME > 0', TO_CHAR(v_count));
