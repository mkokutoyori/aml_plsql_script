...
--   Il interroge en plus les vues du dictionnaire Oracle ALL_TABLES, ALL_VIEWS,
--   ALL_OBJECTS et ALL_TAB_COLUMNS, accessibles a tout utilisateur.
--
-- UN SEUL BLOC, UNE SEULE EXECUTION
--   Tout tient dans un unique bloc PL/SQL anonyme termine par un seul '/'.
--   Chaque section est neanmoins encapsulee dans son propre gestionnaire
--   d'erreur : si une section echoue (objet absent, droit manquant), elle
--   affiche le message et le script poursuit avec la section suivante.
--
-- AUCUNE SAISIE DEMANDEE
--   Le script ne comporte aucune variable de substitution, aucune variable de
--   liaison et aucun caractere esperluette : aucune fenetre de saisie ne doit
--   s'ouvrir au lancement.
--
...
--      Pour l'enregistrer directement dans un fichier, decommenter les deux
--      lignes SPOOL ci-dessous en adaptant le chemin.
--
...
--
-- PLAN DU RAPPORT
--   Partie 1 - sections  1 a  5 : contexte, cartographie du schema, volumetrie
--   Partie 2 - sections  6 a 16 : contrats du module MM
--   Partie 3 - sections 17 a 22 : ecritures comptables ACTB_HISTORY
--   Partie 4 - sections 23 a 30 : CALYPSOUSR, utilisateurs, controles candidats
--
-- DUREE
...
-- SPOOL C:\temp\exploration_mm_report.txt
...
BEGIN
    ...
    po(v_sep);
    po('   PARTIE 1/4 : CONTEXTE ET CARTOGRAPHIE DU SCHEMA');
    po(v_sep);
    ...
    BEGIN
        ...
        FOR r IN (
            SELECT fam, pat FROM (
                SELECT 'LD (Loans and Deposits = support du module MM)' fam, 'LD%'        pat, 1 ord FROM DUAL UNION ALL
    ...
    po(v_sep);
    po('   PARTIE 2/4 : LES CONTRATS DU MODULE MM (LDTB_CONTRACT_MASTER)');
    po(v_sep);
    ...
    BEGIN
        ...
        po('  de ce qui releve du parametrage general du module Loans and Deposits.');
    ...
    BEGIN
        ...
        po(v_sep);
        po('   PARTIE 3/4 : LES ECRITURES COMPTABLES (ACTB_HISTORY)');
        po(v_sep);
    ...
    po(v_sep);
    po('   PARTIE 4/4 : L''APPLICATION CALYPSOUSR, LES UTILISATEURS ET LES CONTROLES');
    po(v_sep);
    ...
    BEGIN
        ...
        FOR u IN (
            ...
            FOR lg IN (SELECT last_signed_on, no_cumulative_logins, no_successive_logins
                ...
                print_kv('  Derniere connexion',         fdth(lg.last_signed_on));
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        DBMS_OUTPUT.PUT_LINE('!! ERREUR GENERALE : ' || SQLERRM);
