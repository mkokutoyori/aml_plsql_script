...
BEGIN
    ...
    -- PERIMETRE : comptes clientelises uniquement. Pour les tests 4.5 et
    --      4.6, le perimetre est applique AVANT le calcul des fonctions
    --      analytiques sur ACTB_ACCBAL_HISTORY (~0,5 M lignes) : le tri
    --      par compte / date ne porte que sur les comptes de la revue.
    -- =========================================================
    ...
    FOR d IN (SELECT * FROM (
        SELECT a.CUST_NO, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO, NVL(a.CCY,'-') AS ccy,
               ...
               COUNT(*) OVER () AS tot_cnt,
               ROW_NUMBER() OVER (ORDER BY a.OVERDRAFT_SINCE ASC) AS rn
        FROM STTM_CUST_ACCOUNT a
        JOIN STTB_ACCOUNT g ON g.AC_GL_NO    = a.CUST_AC_NO
                           AND g.BRANCH_CODE = a.BRANCH_CODE
                           AND g.AC_NATURAL_GL LIKE c_gl_like
    ...
    buf_print('4,12,24,20,5,15,13,9,15',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    buf_print('4,12,24,20,5,15,14,9,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    FOR d IN (SELECT * FROM (
        SELECT a.CUST_NO, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO, NVL(a.CCY,'-') AS ccy,
               ...
               COUNT(*) OVER () AS tot_cnt,
               ROW_NUMBER() OVER (ORDER BY a.TOD_SINCE ASC) AS rn
        FROM STTM_CUST_ACCOUNT a
        JOIN STTB_ACCOUNT g ON g.AC_GL_NO    = a.CUST_AC_NO
                           AND g.BRANCH_CODE = a.BRANCH_CODE
                           AND g.AC_NATURAL_GL LIKE c_gl_like
    ...
    buf_print('4,12,24,20,5,15,15,13,9',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, f.LINE_CODE,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY f.DATE_OF_FIRST_OD ASC) AS rn
            FROM GETM_FACILITY f
            LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
    ...
    buf_print('4,12,24,16,15,15,13,13,9',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',24) || '|'
    ...
    --     Le calcul "gaps and islands" n'est plus execute deux fois : la
    --     requete rapporte a la fois le total et le detail, et ne porte
    --     que sur les soldes journaliers des comptes clientelises.
    buf_reset;
    FOR d IN (
        WITH per_ac AS (
        ...
        bal AS (
            SELECT /*+ MATERIALIZE */ h.ACCOUNT, h.BKG_DATE, h.ACY_CLOSING_BAL
            FROM ACTB_ACCBAL_HISTORY h
            JOIN per_ac p ON p.CUST_AC_NO = h.ACCOUNT
        ...
        plus_long AS (
            ...
            FROM (
                SELECT e.*, ROW_NUMBER() OVER (PARTITION BY ACCOUNT ORDER BY duree DESC) AS rn_ac
                FROM episodes e
            )
            WHERE rn_ac = 1 AND duree > c_jours_long
        )
        SELECT * FROM (
            SELECT x.ACCOUNT, NVL(p.CUST_NO,'-') AS cif, NVL(c.CUSTOMER_NAME1,'-') AS nom,
                   x.deb, x.fin, x.duree, x.pire,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY x.duree DESC) AS rn
            FROM plus_long x
            LEFT JOIN per_ac p ON p.CUST_AC_NO = x.ACCOUNT
            LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = p.CUST_NO
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,12,24,20,13,13,9,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT x.ACCOUNT, NVL(p.CUST_NO,'-') AS cif, NVL(c.CUSTOMER_NAME1,'-') AS nom,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY x.pire ASC) AS rn
            FROM (
                ...
                FROM ACTB_ACCBAL_HISTORY h
                JOIN per_ac pa ON pa.CUST_AC_NO = h.ACCOUNT
                WHERE h.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
                GROUP BY h.ACCOUNT
    ...
    buf_print('4,12,24,20,9,16,16,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, nb_lignes, lim_cum, util_cum, dispo_cum, exp_min,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY util_cum DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                       COUNT(*) AS nb_lignes, NVL(SUM(f.LIMIT_AMOUNT),0) AS lim_cum,
                       NVL(SUM(f.UTILISATION),0) AS util_cum,
                       NVL(SUM(f.AVAILABLE_AMOUNT),0) AS dispo_cum,
    ...
    buf_print('4,12,26,9,16,16,16,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',26) || '|'
    ...
    buf_reset;
    FOR d IN (SELECT * FROM (
        SELECT CUST_NO, nom, nb_cptes, encours, pire, od_depuis, nb_ag,
               COUNT(*) OVER () AS tot_cnt,
               ROW_NUMBER() OVER (ORDER BY encours ASC) AS rn
        FROM (
    ...
    buf_print('4,12,26,10,17,17,13,9',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',26) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, nb_lignes, nb_ag, lim_cum, util_cum, agences,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY lim_cum DESC) AS rn
            FROM (
    ...
    buf_print('4,12,26,9,8,16,16,20',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',26) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, nb_lignes, lim_glob, util_cum, depass, rating,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY depass DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
    ...
    buf_print('4,12,26,9,17,17,17,7',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',26) || '|'
    ...
    --     La sous-requete EXISTS a predicat OR et la sous-requete
    --     scalaire correlee sont remplacees par une jointure unique sur
    --     fac_lim (cle de ligne normalisee, agregee une seule fois).
    buf_reset;
    FOR d IN (
        WITH fac_lim AS (
            SELECT /*+ MATERIALIZE */ line_key, MAX(LIMIT_AMOUNT) AS limite
            FROM (
                SELECT f.LINE_CODE AS line_key, f.LIMIT_AMOUNT
                FROM GETM_FACILITY f WHERE f.LINE_CODE IS NOT NULL
                UNION ALL
                SELECT TO_CHAR(f.ID), f.LIMIT_AMOUNT
                FROM GETM_FACILITY f
                WHERE f.LINE_CODE IS NULL OR TO_CHAR(f.ID) != f.LINE_CODE
        ...
        SELECT * FROM (
            ...
            JOIN STTB_ACCOUNT g ON g.AC_GL_NO    = a.CUST_AC_NO
                               AND g.BRANCH_CODE = a.BRANCH_CODE
                               AND g.AC_NATURAL_GL LIKE c_gl_like
            JOIN fac_lim fl ON fl.line_key = a.LINE_ID
            LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = a.CUST_NO
            WHERE a.RECORD_STAT = 'O'
              AND TRIM(a.LINE_ID) IS NOT NULL
              AND NVL(a.TOD_LIMIT,0) > 0
              AND NVL(fl.limite,0) > 0
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
            || RPAD(' ' || d.CUST_NO,12) || '|' || RPAD(' ' || SUBSTR(d.nom,1,22),24) || '|'
    ...
    buf_print('4,12,24,20,16,15,15,15',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT groupe, nom, nb_liab, nb_lignes, lim_cum, util_cum, lim_groupe,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY util_cum DESC) AS rn
            FROM (
    ...
    buf_print('4,12,26,9,9,17,17,17',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' GROUPE',12) || '|' || RPAD(' NOM GROUPE',26) || '|'
    ...
    -- PERIMETRE : lignes rattachees a un compte clientelise.
    -- PERFORMANCE : le calcul LAG des limites precedentes est isole dans
    --      une CTE materialisee "revisions", restreinte au perimetre et
    --      a la profondeur c_mois_limit. Il n'est plus recalcule deux
    --      fois par test (comptage + detail) mais une seule fois.
    -- =========================================================
    print_section('6. AUGMENTATIONS DE LIMITES IMPORTANTES');

    -- Volumetrie des revisions de limites (informatif)
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('  [Revisions de limites par annee — perimetre clientelise]');
    FOR d IN (
        WITH per_line AS (
            SELECT /*+ MATERIALIZE */ f.ID AS line_id
            FROM GETM_FACILITY f
        ...
        SELECT TO_CHAR(vd.VALUE_DATE,'YYYY') AS annee, COUNT(*) AS nb,
               COUNT(DISTINCT vd.FACILITY_ID) AS nb_lignes
        FROM GETM_FACILITY_VD_DETAILS vd
        WHERE vd.VALUE_DATE IS NOT NULL
          AND vd.FACILITY_ID IN (SELECT line_id FROM per_line)
        GROUP BY TO_CHAR(vd.VALUE_DATE,'YYYY')
    ...
    FOR d IN (
        ...
        , revisions AS (
            SELECT /*+ MATERIALIZE */ vd.ID, vd.FACILITY_ID, vd.LIMIT_AMOUNT,
                   vd.VALUE_DATE, vd.BOOK_DATE,
        ...
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                   ...
                   v.VALUE_DATE,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY v.LIMIT_AMOUNT - v.lim_prec DESC) AS rn
            FROM revisions v
    ...
    buf_print('4,12,22,16,16,16,16,8,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
    ...
    buf_print('4,12,22,16,16,16,16,12,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
        || RPAD(' LIGNE',16) || '|' || RPAD(' ANCIENNE LIM.',16) || '|' || RPAD(' NOUVELLE LIM.',16) || '|'
    ...
    buf_print('4,12,22,16,16,16,10,12,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
        || RPAD(' LIGNE',16) || '|' || RPAD(' ANCIENNE LIM.',16) || '|' || RPAD(' NOUVELLE LIM.',16) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, ligne, nb_hausses, lim_init, lim_fin, hausse_tot, derniere,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY nb_hausses DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                       NVL(f.LINE_CODE, TO_CHAR(v.FACILITY_ID)) AS ligne,
                       COUNT(*) AS nb_hausses,
    ...
    buf_print('4,12,22,16,10,16,16,16,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
    ...
    buf_print('4,12,22,16,16,16,16,9,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, f.LINE_CODE,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY NVL(f.MOD_NO,0) DESC) AS rn
            FROM GETM_FACILITY f
            LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
    ...
    buf_print('4,12,24,16,8,16,16,16,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',24) || '|'
    ...
    -- PERIMETRE : couples (compte clientelise, ligne rattachee), obtenus
    --      une seule fois par la CTE materialisee per_ac_line ; celle-ci
    --      remplace la jointure a predicat OR
    --      (a.LINE_ID = f.LINE_CODE OR a.LINE_ID = TO_CHAR(f.ID)) par
    --      deux equi-jointures.
    -- PERFORMANCE : les sous-requetes EXISTS et scalaires correlees sur
    --      ACTB_ACCBAL_HISTORY / ACTB_HISTORY, evaluees ligne a ligne,
    --      sont remplacees par une jointure unique sur un jeu de travail
    --      materialise et agrege (le pire solde et le plus gros debit
    --      sont calcules dans la meme passe que le test lui-meme).
    -- =========================================================
    ...
    FOR d IN (
        WITH per_ac_line AS (
            SELECT /*+ MATERIALIZE */ a.CUST_AC_NO, f.ID AS line_id
            FROM STTM_CUST_ACCOUNT a
            JOIN STTB_ACCOUNT g ON g.AC_GL_NO = a.CUST_AC_NO
                               AND g.BRANCH_CODE = a.BRANCH_CODE
            JOIN GETM_FACILITY f ON f.LINE_CODE = a.LINE_ID
            WHERE g.AC_NATURAL_GL LIKE c_gl_like
            UNION
            SELECT a.CUST_AC_NO, f.ID
            FROM STTM_CUST_ACCOUNT a
            JOIN STTB_ACCOUNT g ON g.AC_GL_NO = a.CUST_AC_NO
                               AND g.BRANCH_CODE = a.BRANCH_CODE
            JOIN GETM_FACILITY f ON TO_CHAR(f.ID) = a.LINE_ID
            WHERE g.AC_NATURAL_GL LIKE c_gl_like
        )
        , revisions AS (
            SELECT /*+ MATERIALIZE */ vd.ID, vd.FACILITY_ID, vd.LIMIT_AMOUNT,
            ...
            FROM GETM_FACILITY_VD_DETAILS vd
            WHERE vd.VALUE_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_limit)
              AND vd.FACILITY_ID IN (SELECT line_id FROM per_ac_line)
        )
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                   NVL(f.LINE_CODE, TO_CHAR(v.FACILITY_ID)) AS ligne,
    ...
    buf_print('4,12,22,16,16,16,12,12,8',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
        || RPAD(' LIGNE',16) || '|' || RPAD(' ANCIENNE LIM.',16) || '|' || RPAD(' NOUVELLE LIM.',16) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY TRUNC(v.BOOK_DATE) - TRUNC(v.VALUE_DATE) DESC) AS rn
            FROM revisions v
            LEFT JOIN GETM_FACILITY f ON f.ID = v.FACILITY_ID
            LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
            WHERE v.lim_prec IS NOT NULL AND v.LIMIT_AMOUNT > v.lim_prec
    ...
    buf_print('4,12,22,16,16,16,12,12,9',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
        || RPAD(' LIGNE',16) || '|' || RPAD(' ANCIENNE LIM.',16) || '|' || RPAD(' NOUVELLE LIM.',16) || '|'
    ...
    FOR d IN (
        ...
        , bal AS (
            SELECT /*+ MATERIALIZE */ h.ACCOUNT, h.BKG_DATE, h.ACY_CLOSING_BAL
            FROM ACTB_ACCBAL_HISTORY h
            WHERE h.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_limit) - c_jours_avant
              AND h.ACCOUNT IN (SELECT CUST_AC_NO FROM per_ac_line)
        )
        SELECT * FROM (
            SELECT liab, nom, CUST_AC_NO, ancienne, nouvelle, pire, VALUE_DATE,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY nouvelle - ancienne DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, p.CUST_AC_NO,
                       v.lim_prec AS ancienne, v.LIMIT_AMOUNT AS nouvelle,
                       MIN(b.ACY_CLOSING_BAL) AS pire, v.VALUE_DATE
                FROM revisions v
                JOIN per_ac_line p ON p.line_id = v.FACILITY_ID
                JOIN GETM_FACILITY f ON f.ID = v.FACILITY_ID
                LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
                JOIN bal b ON b.ACCOUNT = p.CUST_AC_NO
                          AND b.BKG_DATE BETWEEN v.VALUE_DATE - c_jours_avant AND v.VALUE_DATE
                WHERE v.lim_prec IS NOT NULL AND v.LIMIT_AMOUNT > v.lim_prec
                GROUP BY v.ID, l.LIAB_NO, l.LIAB_NAME, p.CUST_AC_NO,
                         v.lim_prec, v.LIMIT_AMOUNT, v.VALUE_DATE
                HAVING MIN(b.ACY_CLOSING_BAL) < -v.lim_prec
            )
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,12,20,20,15,15,15,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',20) || '|'
    ...
    FOR d IN (
        ...
        , bal AS (
            SELECT /*+ MATERIALIZE */ h.ACCOUNT, h.BKG_DATE, h.ACY_CLOSING_BAL
            FROM ACTB_ACCBAL_HISTORY h
            WHERE h.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_limit)
              AND h.ACCOUNT IN (SELECT CUST_AC_NO FROM per_ac_line)
        )
        SELECT * FROM (
            SELECT liab, nom, CUST_AC_NO, ancienne, nouvelle, pire, VALUE_DATE,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY nouvelle DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, p.CUST_AC_NO,
                       v.lim_prec AS ancienne, v.LIMIT_AMOUNT AS nouvelle,
                       MIN(b.ACY_CLOSING_BAL) AS pire, v.VALUE_DATE
                ...
                LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
                JOIN bal b ON b.ACCOUNT = p.CUST_AC_NO
                          AND b.BKG_DATE BETWEEN v.VALUE_DATE AND v.VALUE_DATE + c_jours_avant
                WHERE v.lim_prec IS NOT NULL AND v.LIMIT_AMOUNT > v.lim_prec
                GROUP BY v.ID, l.LIAB_NO, l.LIAB_NAME, p.CUST_AC_NO,
                         v.lim_prec, v.LIMIT_AMOUNT, v.VALUE_DATE
                HAVING MIN(b.ACY_CLOSING_BAL) <= -0.9 * v.LIMIT_AMOUNT
            )
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    FOR d IN (
        ...
        , deb AS (
            SELECT /*+ MATERIALIZE */ h.AC_NO, h.TRN_DT, h.LCY_AMOUNT
            FROM ACTB_HISTORY h
            WHERE h.DRCR_IND = 'D'
              AND h.TRN_DT >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_limit)
              AND h.AC_NO IN (SELECT CUST_AC_NO FROM per_ac_line)
        )
        SELECT * FROM (
            SELECT liab, nom, CUST_AC_NO, ancienne, nouvelle, nb_deb, gros_deb, VALUE_DATE,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY nouvelle - ancienne DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, p.CUST_AC_NO,
                       v.lim_prec AS ancienne, v.LIMIT_AMOUNT AS nouvelle,
                       COUNT(CASE WHEN h.LCY_AMOUNT >= c_mnt_signif THEN 1 END) AS nb_deb,
                       MAX(h.LCY_AMOUNT) AS gros_deb, v.VALUE_DATE
                FROM revisions v
                JOIN per_ac_line p ON p.line_id = v.FACILITY_ID
                JOIN GETM_FACILITY f ON f.ID = v.FACILITY_ID
                LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
                JOIN deb h ON h.AC_NO = p.CUST_AC_NO
                          AND h.TRN_DT BETWEEN v.VALUE_DATE AND v.VALUE_DATE + c_jours_avant
                WHERE v.lim_prec IS NOT NULL AND v.LIMIT_AMOUNT > v.lim_prec
                GROUP BY v.ID, l.LIAB_NO, l.LIAB_NAME, p.CUST_AC_NO,
                         v.lim_prec, v.LIMIT_AMOUNT, v.VALUE_DATE
                HAVING COUNT(CASE WHEN h.LCY_AMOUNT >= c_mnt_signif THEN 1 END) > 0
            )
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,12,20,20,15,15,9,15,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',20) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' ANCIENNE LIM.',15) || '|' || RPAD(' NOUVELLE LIM.',15) || '|'
    ...
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('  [Ventilation des lignes du perimetre par palier de delegation]');
    FOR d IN (
        WITH per_line AS (
            SELECT /*+ MATERIALIZE */ f.ID AS line_id
            FROM GETM_FACILITY f
        ...
        SELECT palier, COUNT(*) AS nb, NVL(SUM(mnt),0) AS total,
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, ligne, ccy, mnt, seuil,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY mnt DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
    ...
    buf_print('4,12,22,16,5,16,16,10,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
        || RPAD(' LIGNE',16) || '|' || RPAD(' CCY',5) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, ligne, mnt, approuve, util, checker, CHECKER_DT_STAMP,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY mnt DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                       f.LINE_CODE AS ligne,
                       CASE WHEN NVL(f.LINE_CURRENCY,'XAF') = 'XAF'
                            THEN NVL(f.LIMIT_AMOUNT,0)
                            ELSE NVL(NULLIF(f.REPORTING_AMOUNT,0), NVL(f.LIMIT_AMOUNT,0))
                       END AS mnt,
    ...
    buf_print('4,12,22,16,16,16,16,16,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',22) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, nb_lignes, cumul, plus_grosse, util_cum,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY cumul DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
                       COUNT(*) AS nb_lignes,
                       SUM(NVL(f.LIMIT_AMOUNT,0)) AS cumul,
                       MAX(NVL(f.LIMIT_AMOUNT,0)) AS plus_grosse,
                       NVL(SUM(f.UTILISATION),0) AS util_cum
                FROM GETM_FACILITY f
                LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
                WHERE f.RECORD_STAT = 'O' AND NVL(f.LIMIT_AMOUNT,0) > 0
    ...
    buf_print('4,12,24,10,17,17,17,17',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',24) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT liab, nom, ligne, nb_hausses, cumul, plus_grosse, lim_fin, derniere,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY cumul DESC) AS rn
            FROM (
                SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom,
    ...
    buf_print('4,12,24,20,5,16,16,10,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',24) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    -- PERIMETRE : comptes clientelises uniquement.
    -- PERFORMANCE : tous les tests de la section partagent le meme jeu de
    --      travail (ecritures DE de la periode sur comptes clientelises
    --      debiteurs). Ce jeu est isole dans la CTE materialisee "de" :
    --      la jointure ACTB_HISTORY x ACTB_ACCBAL_HISTORY, auparavant
    --      evaluee 11 fois (2 par test + la cartographie), n'est plus
    --      evaluee qu'une fois par requete, soit 6 fois, et sur un
    --      volume reduit par le filtre de perimetre applique en amont.
    -- =========================================================
    ...
    FOR d IN (
        ...
        , de AS (
            -- Jeu de travail commun a la section : ecritures manuelles
            -- (module DE) de la periode, passees sur un compte
            -- clientelise dont le solde de cloture du jour est negatif.
            SELECT /*+ MATERIALIZE */ h.AC_NO, p.CUST_NO, h.TRN_REF_NO, h.DRCR_IND,
                   h.TRN_CODE, h.LCY_AMOUNT, h.TRN_DT, h.VALUE_DT,
                   h.USER_ID, h.AUTH_ID, b.ACY_CLOSING_BAL AS solde
            FROM ACTB_HISTORY h
            JOIN per_ac p ON p.CUST_AC_NO = h.AC_NO
        ...
        SELECT * FROM (
            SELECT AC_NO, cif, nom, nb_ecr, tot_deb, tot_cred, nb_users, derniere,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY tot_mvt DESC) AS rn
            FROM (
                SELECT h.AC_NO, NVL(MAX(h.CUST_NO),'-') AS cif,
                       NVL(MAX(c.CUSTOMER_NAME1),'-') AS nom,
                       COUNT(*) AS nb_ecr,
                       ...
                       SUM(h.LCY_AMOUNT) AS tot_mvt
                FROM de h
                LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = h.CUST_NO
                GROUP BY h.AC_NO
            )
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
    ...
    buf_print('4,12,22,20,8,16,16,9,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT h.AC_NO, h.TRN_REF_NO, h.DRCR_IND, NVL(h.TRN_CODE,'-') AS trn_code,
                   h.LCY_AMOUNT, h.TRN_DT, NVL(h.USER_ID,'-') AS usr, h.solde,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY h.LCY_AMOUNT DESC) AS rn
            FROM de h
            WHERE h.LCY_AMOUNT >= c_mnt_signif
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,20,12,4,7,16,12,16,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' COMPTE',20) || '|' || RPAD(' REFERENCE',12) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT h.AC_NO, h.TRN_REF_NO, h.DRCR_IND, h.LCY_AMOUNT, h.TRN_DT,
                   h.USER_ID AS usr, h.solde,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY h.LCY_AMOUNT DESC) AS rn
            FROM de h
            WHERE h.USER_ID IS NOT NULL AND h.AUTH_ID IS NOT NULL
    ...
    buf_print('4,20,12,4,16,12,18,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' COMPTE',20) || '|' || RPAD(' REFERENCE',12) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT h.AC_NO, h.TRN_REF_NO, h.DRCR_IND, h.LCY_AMOUNT, h.TRN_DT, h.VALUE_DT,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY TRUNC(h.TRN_DT) - TRUNC(h.VALUE_DT) DESC,
                                               h.LCY_AMOUNT DESC) AS rn
            FROM de h
            WHERE h.VALUE_DT IS NOT NULL
    ...
    buf_print('4,20,12,4,16,12,12,8,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' COMPTE',20) || '|' || RPAD(' REFERENCE',12) || '|'
    ...
    --     La sous-requete scalaire correlee a predicat OR sur
    --     GETM_FACILITY (evaluee pour chaque ecriture) est remplacee par
    --     une jointure sur fac_lim, agregee une seule fois.
    buf_reset;
    FOR d IN (
        WITH per_ac AS (
            SELECT /*+ MATERIALIZE */ a.CUST_AC_NO, a.CUST_NO, a.LINE_ID,
        ...
        , autor AS (
            SELECT /*+ MATERIALIZE */ p.CUST_AC_NO, p.CUST_NO,
                   NVL(fl.limite,0) + p.tod_limit AS autorise
            FROM per_ac p
            LEFT JOIN fac_lim fl ON fl.line_key = p.LINE_ID
        )
        SELECT * FROM (
            SELECT au.CUST_NO, h.AC_NO, h.TRN_REF_NO, h.LCY_AMOUNT, h.TRN_DT,
                   b.ACY_CLOSING_BAL AS solde, NVL(h.USER_ID,'-') AS usr,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY h.LCY_AMOUNT DESC) AS rn
            FROM ACTB_HISTORY h
            JOIN autor au ON au.CUST_AC_NO = h.AC_NO
            JOIN ACTB_ACCBAL_HISTORY b ON b.ACCOUNT = h.AC_NO
                 AND b.BKG_DATE = TRUNC(h.TRN_DT)
            WHERE h.MODULE = 'DE' AND h.DRCR_IND = 'D'
              AND h.TRN_DT >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
              AND b.ACY_CLOSING_BAL < -au.autorise
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,12,20,12,16,12,16,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' COMPTE',20) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT usr, nom, nb_ecr, nb_cptes, tot_deb, tot_cred,
                   ROW_NUMBER() OVER (ORDER BY nb_ecr DESC) AS rn
            FROM (
    ...
    v_count := v_row_num;
    IF v_row_num = 0 THEN
        DBMS_OUTPUT.PUT_LINE('  (aucune ecriture manuelle sur compte clientelise debiteur)');
    ELSE
        buf_print('4,18,26,9,9,17,17',
            '  |' || RPAD(' N#',4) || '|' || RPAD(' USER_ID',18) || '|' || RPAD(' NOM UTILISATEUR',26) || '|'
    ...
    -- prudentiel.
    -- PERIMETRE : comptes clientelises uniquement, applique avant tout
    --      acces aux historiques.
    -- =========================================================
    ...
    FOR d IN (
        ...
        , bal AS (
            SELECT /*+ MATERIALIZE */ b.ACCOUNT, b.BKG_DATE, b.ACY_CLOSING_BAL
            FROM ACTB_ACCBAL_HISTORY b
            JOIN per_ac p ON p.CUST_AC_NO = b.ACCOUNT
            WHERE b.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
        )
        SELECT * FROM (
            SELECT NVL(p.CUST_NO,'-') AS cif, x.ACCOUNT, x.BKG_DATE, x.bal_prec,
                   x.ACY_CLOSING_BAL AS bal_arrete, x.bal_suiv, x.dt_suiv,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY x.bal_prec ASC) AS rn
            FROM (
                SELECT ACCOUNT, BKG_DATE, ACY_CLOSING_BAL,
    ...
    buf_print('4,12,20,12,16,16,16,12',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' COMPTE',20) || '|'
    ...
    --      L'auto-jointure d'ACTB_HISTORY (~5,8 M lignes de chaque cote)
    --      est remplacee par la jointure de deux jeux de travail
    --      materialises et deja filtres :
    --        - cred : credits >= c_mnt_signif sur compte clientelise
    --                 debiteur dans les 5 jours precedents ;
    --        - deb  : debits >= 95 % de c_mnt_signif (borne deduite de
    --                 la tolerance de 5 % sur le montant du retour).
    buf_reset;
    FOR d IN (
        WITH per_ac AS (
            SELECT /*+ MATERIALIZE */ a.CUST_AC_NO, a.CUST_NO
        ...
        , bal_neg AS (
            SELECT /*+ MATERIALIZE */ DISTINCT b.ACCOUNT, b.BKG_DATE
            FROM ACTB_ACCBAL_HISTORY b
            JOIN per_ac p ON p.CUST_AC_NO = b.ACCOUNT
            WHERE b.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist) - 5
              AND b.ACY_CLOSING_BAL < 0
        )
        , cred AS (
            SELECT /*+ MATERIALIZE */ h.AC_NO, h.TRN_DT, h.LCY_AMOUNT, h.MODULE
            FROM ACTB_HISTORY h
            JOIN per_ac p ON p.CUST_AC_NO = h.AC_NO
            WHERE h.DRCR_IND = 'C'
              AND h.LCY_AMOUNT >= c_mnt_signif
              AND h.TRN_DT >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
              AND EXISTS (SELECT 1 FROM bal_neg n
                          WHERE n.ACCOUNT = h.AC_NO
                            AND n.BKG_DATE BETWEEN TRUNC(h.TRN_DT) - 5 AND TRUNC(h.TRN_DT))
        )
        , deb AS (
            SELECT /*+ MATERIALIZE */ h.AC_NO, h.TRN_DT, h.LCY_AMOUNT, h.MODULE
            FROM ACTB_HISTORY h
        ...
        SELECT * FROM (
            SELECT hc.AC_NO, hc.LCY_AMOUNT AS mnt_cred, hc.TRN_DT AS dt_cred,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY hc.LCY_AMOUNT DESC) AS rn
            FROM cred hc
            JOIN deb hd ON hd.AC_NO = hc.AC_NO
    ...
    buf_print('4,20,16,12,16,12,7,7',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' COMPTE',20) || '|' || RPAD(' CREDIT',16) || '|'
    ...
    buf_print('4,20,12,7,16,12,16,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' COMPTE',20) || '|' || RPAD(' REFERENCE',12) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(p.CUST_NO,'-') AS cif, h.AC_NO, h.TRN_REF_NO, NVL(h.MODULE,'-') AS mdl,
    ...
    buf_print('4,12,20,12,7,16,12,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' COMPTE',20) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT p.CUST_NO, h.AC_NO, h.RELATED_ACCOUNT, h.LCY_AMOUNT, h.TRN_DT,
            ...
            WHERE h.DRCR_IND = 'C'
              ...
              AND a2.CUST_NO = p.CUST_NO
              AND a2.CUST_AC_NO != h.AC_NO
    ...
    buf_print('4,12,20,20,16,12,7,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' COMPTE CREDITE',20) || '|'
    ...
    -- PERIMETRE : comptes clientelises uniquement.
    -- PERFORMANCE : les ecritures d'interets debiteurs (module IC) sont
    --      agregees une seule fois par compte dans la CTE materialisee
    --      "ic_ac". Les sous-requetes NOT EXISTS et scalaires correlees
    --      sur ACTB_HISTORY, evaluees pour chaque compte, deviennent une
    --      simple jointure externe (tests 11.1 et 11.6).
    -- =========================================================
    ...
    FOR d IN (
        ...
        , ic_ac AS (
            -- Interets debiteurs (module IC) effectivement debites sur la
            -- periode, agreges une seule fois par compte du perimetre.
            SELECT /*+ MATERIALIZE */ h.AC_NO, SUM(h.LCY_AMOUNT) AS interets
            FROM ACTB_HISTORY h
            JOIN per_ac p ON p.CUST_AC_NO = h.AC_NO
        ...
        SELECT * FROM (
            SELECT a.CUST_NO, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO, NVL(a.CCY,'-') AS ccy,
                   ...
                   ROW_NUMBER() OVER (ORDER BY a.LCY_CURR_BALANCE ASC) AS rn
            FROM STTM_CUST_ACCOUNT a
            JOIN per_ac p ON p.CUST_AC_NO = a.CUST_AC_NO
            LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = a.CUST_NO
            WHERE a.RECORD_STAT = 'O'
              AND NVL(a.ACY_CURR_BALANCE,0) < 0
              AND a.OVERDRAFT_SINCE IS NOT NULL
              AND TRUNC(SYSDATE) - TRUNC(a.OVERDRAFT_SINCE) > c_jours_tod_max
              AND NOT EXISTS (SELECT 1 FROM ic_ac i WHERE i.AC_NO = a.CUST_AC_NO)
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
    ...
    buf_print('4,12,22,20,5,16,13,9,14',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    buf_print('4,12,22,20,5,16,16,16,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    FOR d IN (SELECT * FROM (
        SELECT a.CUST_NO, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO, NVL(a.CCY,'-') AS ccy,
               ...
               a.ACY_CURR_BALANCE AS solde, a.OVERDRAFT_SINCE,
               COUNT(*) OVER () AS tot_cnt,
               ROW_NUMBER() OVER (ORDER BY NVL(a.DR_INT_DUE,0) DESC) AS rn
        FROM STTM_CUST_ACCOUNT a
        JOIN STTB_ACCOUNT g ON g.AC_GL_NO    = a.CUST_AC_NO
                           AND g.BRANCH_CODE = a.BRANCH_CODE
                           AND g.AC_NATURAL_GL LIKE c_gl_like
    ...
    FOR d IN (SELECT * FROM (
        SELECT a.CUST_NO, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO, NVL(a.CCY,'-') AS ccy,
               ...
               COUNT(*) OVER () AS tot_cnt,
               ROW_NUMBER() OVER (ORDER BY NVL(a.CHG_DUE,0) DESC) AS rn
        FROM STTM_CUST_ACCOUNT a
        JOIN STTB_ACCOUNT g ON g.AC_GL_NO    = a.CUST_AC_NO
                           AND g.BRANCH_CODE = a.BRANCH_CODE
                           AND g.AC_NATURAL_GL LIKE c_gl_like
    ...
    buf_print('4,12,22,20,5,16,16,13,14',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    buf_print('4,12,22,20,16,12,24,7,7',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
    ...
    FOR d IN (
        ...
        , enc AS (
            SELECT /*+ MATERIALIZE */ b.ACCOUNT,
                   AVG(ABS(b.ACY_CLOSING_BAL)) AS enc_moy, COUNT(*) AS nb_j
            FROM ACTB_ACCBAL_HISTORY b
            JOIN per_ac p ON p.CUST_AC_NO = b.ACCOUNT
            WHERE b.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
              AND b.ACY_CLOSING_BAL < 0
        ...
        SELECT * FROM (
            SELECT NVL(p.CUST_NO,'-') AS cif, s.ACCOUNT, s.nb_j, s.enc_moy,
                   NVL(i.interets,0) AS interets,
                   ROUND(NVL(i.interets,0) * 365 * 100 / NULLIF(s.enc_moy * s.nb_j, 0), 2) AS taux,
                   c_taux_od_min / 100 * s.enc_moy * s.nb_j / 365 AS attendus,
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY s.enc_moy DESC) AS rn
            FROM enc s
            LEFT JOIN ic_ac i ON i.AC_NO = s.ACCOUNT
            LEFT JOIN per_ac p ON p.CUST_AC_NO = s.ACCOUNT
            WHERE NVL(i.interets,0) < c_taux_od_min / 100 * s.enc_moy * s.nb_j / 365
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
            || RPAD(' ' || d.cif,12) || '|' || RPAD(' ' || d.ACCOUNT,20) || '|'
    ...
    buf_print('4,12,20,9,17,16,10,16',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' COMPTE',20) || '|'
    ...
    -- PERIMETRE : comptes clientelises uniquement.
    -- PERFORMANCE : le comptage des episodes (LAG sur les soldes
    --      journaliers) est restreint au perimetre et n'est plus
    --      execute deux fois par test. La synthese 12.6, qui evaluait
    --      quatre sous-requetes correlees PAR COMPTE (dont deux sur
    --      ACTB_HISTORY et deux a predicat OR sur GETM_FACILITY), repose
    --      desormais sur des jeux de reference materialises une fois.
    -- =========================================================
    ...
    FOR d IN (
        ...
        , episodes AS (
            -- Episodes de decouvert par compte du perimetre : un nouvel
            -- episode debute des que le solde repasse sous zero apres un
            -- retour a l'equilibre.
            SELECT /*+ MATERIALIZE */ ACCOUNT,
                   SUM(CASE WHEN NVL(bal_prec,0) >= 0 THEN 1 ELSE 0 END) AS nb_ep,
                   COUNT(*) AS nb_j, MIN(ACY_CLOSING_BAL) AS pire
            FROM (
                SELECT b.ACCOUNT, b.BKG_DATE, b.ACY_CLOSING_BAL,
                       LAG(b.ACY_CLOSING_BAL) OVER (PARTITION BY b.ACCOUNT
                            ORDER BY b.BKG_DATE) AS bal_prec
                FROM ACTB_ACCBAL_HISTORY b
                JOIN per_ac p ON p.CUST_AC_NO = b.ACCOUNT
                WHERE b.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
            )
        ...
        SELECT * FROM (
            SELECT NVL(a.CUST_NO,'-') AS cif, NVL(c.CUSTOMER_NAME1,'-') AS nom, ep.ACCOUNT,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY ep.nb_ep DESC, ep.nb_j DESC) AS rn
            FROM episodes ep
            LEFT JOIN STTM_CUST_ACCOUNT a ON a.CUST_AC_NO = ep.ACCOUNT
            LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = a.CUST_NO
            WHERE ep.nb_ep >= c_nb_episodes
        ) WHERE rn <= c_max_rows ORDER BY rn) LOOP
        v_count := d.tot_cnt;
        buf_add('  |' || LPAD(d.rn,3) || ' |'
            || RPAD(' ' || d.cif,12) || '|' || RPAD(' ' || SUBSTR(d.nom,1,20),22) || '|'
    ...
    buf_print('4,12,22,20,10,10,17,17',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(a.CUST_NO,'-') AS cif, NVL(c.CUSTOMER_NAME1,'-') AS nom, s.ACCOUNT,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY s.nb_j DESC, s.pire ASC) AS rn
            FROM (
                SELECT b.ACCOUNT, COUNT(*) AS nb_j, AVG(b.ACY_CLOSING_BAL) AS moyen,
                       MIN(b.ACY_CLOSING_BAL) AS pire
                FROM ACTB_ACCBAL_HISTORY b
                JOIN per_ac p ON p.CUST_AC_NO = b.ACCOUNT
                WHERE b.BKG_DATE >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
                  AND b.ACY_CLOSING_BAL < 0
    ...
    buf_print('4,12,22,20,10,17,17,17',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
    ...
    buf_print('4,12,22,20,5,16,13,13,10',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
        || RPAD(' COMPTE',20) || '|' || RPAD(' CCY',5) || '|'
    ...
    buf_print('4,12,22,20,16,16,13,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
    ...
    FOR d IN (
        ...
        SELECT * FROM (
            SELECT NVL(l.LIAB_NO,'-') AS liab, NVL(l.LIAB_NAME,'-') AS nom, f.LINE_CODE,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY NVL(f.EXCEP_BREACH,0) DESC) AS rn
            FROM GETM_FACILITY f
            LEFT JOIN GETM_LIAB l ON l.ID = f.LIAB_ID
    ...
    buf_print('4,12,24,16,10,16,16,13,13',
        '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF/LIAB',12) || '|' || RPAD(' NOM',24) || '|'
    ...
    FOR d IN (
        ...
        , ic_ac AS (
            SELECT /*+ MATERIALIZE */ DISTINCT h.AC_NO
            FROM ACTB_HISTORY h
            JOIN per_ac p ON p.CUST_AC_NO = h.AC_NO
            WHERE h.MODULE = 'IC' AND h.DRCR_IND = 'D'
              AND h.TRN_DT >= ADD_MONTHS(TRUNC(SYSDATE), -c_mois_hist)
        ...
        , fac_lim AS (
            ...
            FROM (
                SELECT f.LINE_CODE AS line_key, f.LIMIT_AMOUNT
                FROM GETM_FACILITY f
                WHERE f.RECORD_STAT = 'O' AND f.LINE_CODE IS NOT NULL
                UNION ALL
                SELECT TO_CHAR(f.ID), f.LIMIT_AMOUNT
                FROM GETM_FACILITY f
                WHERE f.RECORD_STAT = 'O'
                  AND (f.LINE_CODE IS NULL OR TO_CHAR(f.ID) != f.LINE_CODE)
            )
            GROUP BY line_key
        )
        , fac_exp AS (
        ...
        SELECT * FROM (
            SELECT cif, nom, compte, solde, fa, fb, fc, fd, fe, ff,
                   ...
                   COUNT(*) OVER () AS tot_cnt,
                   ROW_NUMBER() OVER (ORDER BY fa + fb + fc + fd + fe + ff DESC, solde ASC) AS rn
            FROM (
                SELECT a.CUST_NO AS cif, NVL(c.CUSTOMER_NAME1,'-') AS nom, a.CUST_AC_NO AS compte,
                       ...
                       CASE WHEN ABS(a.ACY_CURR_BALANCE) >
                                 NVL(fl.limite,0) + NVL(a.TOD_LIMIT,0)
                       ...
                       CASE WHEN fe2.line_key IS NOT NULL THEN 1 ELSE 0 END AS ff
                FROM STTM_CUST_ACCOUNT a
                JOIN per_ac p ON p.CUST_AC_NO = a.CUST_AC_NO
                LEFT JOIN STTM_CUSTOMER c ON c.CUSTOMER_NO = a.CUST_NO
                LEFT JOIN episodes ep ON ep.ACCOUNT = a.CUST_AC_NO
                LEFT JOIN ic_ac ic ON ic.AC_NO = a.CUST_AC_NO
                LEFT JOIN de_ac de ON de.AC_NO = a.CUST_AC_NO
                LEFT JOIN fac_lim fl ON fl.line_key = a.LINE_ID
                LEFT JOIN fac_exp fe2 ON fe2.line_key = a.LINE_ID
                WHERE a.RECORD_STAT = 'O' AND NVL(a.ACY_CURR_BALANCE,0) < 0
            )
    ...
    ELSE
        buf_print('4,12,22,20,17,7,7,7,7,7,7,7',
            '  |' || RPAD(' N#',4) || '|' || RPAD(' CIF',12) || '|' || RPAD(' NOM CLIENT',22) || '|'
